...
if __name__ == "__main__":
  parser = argparse.ArgumentParser(description="Generates csv compatible with Anki from list of words")
  parser.add_argument("words", help="file with list of words")
  parser.add_argument("app_id", help="ID of Oxford Dictionaries registered app")
  parser.add_argument("app_key", help="Key of Oxford Dictionaries registered app")
  parser.add_argument("--csv", nargs=1, help="Name of export FILE", metavar="FILE", default=["cards.csv"])
  parser.add_argument("--rename", help="Rename mp3 files from Audacity numbers to names", action="store_true")
  ...
  for num, word in enumerate(words, start = 1):
    ...
    if num % 2 == 1:
      ...
      cards_csv.writerow([prev, "[sound:{}.mp3]".format(prev), ipa(prev, header), to_es(prev, header)]
                         + [word, "[sound:{}.mp3]".format(word), ipa(word, header), to_es(word, header)])
