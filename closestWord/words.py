...
def closest(query, dictionary, n):
  ...
  return sorted(
    dictionary, key = lambda word: editdistance.eval(query, word))[:n]


if __name__ == "__main__":
  parser = argparse.ArgumentParser(
    description =
    "Finds list of words closest to a given one using edit distance")
  ...
  try:
    ...
    if os.path.exists(args.word):
      print(
        ", but '{}' is.\nDid you write the arguments in the wrong order?"
        .format(args.word))
    else:
      print("")
    exit(-1)
