...
if __name__ == "__main__":
  parser = argparse.ArgumentParser(description="Finds list of words closest to a given one using edit distance")
  parser.add_argument("dictionary", help="List of words (one per line)")
  parser.add_argument("word", help="Query word")
  parser.add_argument("--n", nargs=1, help="Number of results", metavar="NUM", default=['10'])
  ...
  try:
    ...
    if os.path.exists(args.word):
      print(", but '{}' is.\nDid you write the arguments in the wrong order?".format(args.word))
    else:
      print("")
    exit(-1)
